<?php
    $menu = product_discount()['menu'];
    $items = product_discount()['items'];
    $count = count($items);
...
<style>
    .ribbon-new:after {
        ...
        content: " ";
        background-image: url(https://resource.natrocdn.com/ResourceFiles/v1/images/default/ribbon-new.png);
...
<div class="content clearfix" id="wt_refpoint">
    <div id="category">
        <h2>
            <ul class="breadcrumb">
                <li>
                    <a href="#" title="">İndirimli Ürünler</a>
            ...
            <ul id="subnavi" class="nav nav-tabs">
				<?php foreach ($menu as $key=>$val):?>
                    ...
                    <li class="has-subnavi" >
                        <a class="btn-catitem long" href="<?php if($val['alone'] == 0){echo shop_url('product-view?id='.$val['id']);} else{echo URI::get_path('product-views?id='.$val['id']);} ?>">
                                                ...
                                                <a class="item-thumb fancybox fancybox.ajax" href="<?=shop_url('product-ajax?id='.$val['item_id'].'&token='.$itemToken)?>">
                                                    <div class="item-thumb-container">
                                                        <div id="image" class="picture_wrapper_2">
                                                            <img class="item-thumb-63" src="<?= URL . '/' .$val['item_image']?>" alt="<?=$val['item_name']?>" onerror="this.src='<?=URL.'/data/items/60001.png'?>'">                                                        </div>
                                                    </div>
                                                </a>
