...
// güncellendi mi test
$menu = product_views(get('id'))['menus'];
$allMenu = product_views(get('id'))['allMenu'];
        ...
        <div class="tabbable tabs-left">
            <ul id="subnavi" class="nav nav-tabs">
				<?php foreach ($menu as $key=>$val):?>
                    <li class="has-subnavi" >
                        <a class="btn-catitem btn-catitem<?php if($getMenuId == $val['who']){echo '-active';}?>" href="<?php if($val['alone'] == 0){echo shop_url('product-views?id='.$val['id']);} else{echo shop_url('product-views?id='.$val['id']);} ?>">
						...
						<?php if ($getSubMenu->rowCount() > 0):?>
                            <ul class="dropdown-menu ">
								<?php foreach ($subMenu as $keys=>$rows):?>
                                    ...
                                    <li class="has-subnavi">
                                        <a class="btn-catitem btn-catitem-active" href="<?=shop_url('product-view?id='.$rows['id'])?>"><?=$rows['name'];?></a>
                                    </li>
								<?php endforeach;?>
                            </ul>
						<?php endif;?>
            ...
            <div class="tab-content">
                <div class="scrollable_container row-fluid">
					...
					$items = $db->prepare("SELECT * FROM items WHERE kategori_id = :kategori_id AND durum = :status");
                    ...
                    <ul class="item-list card clearfix">
							<?php foreach ($items->fetchAll() as $key=>$val):?>
