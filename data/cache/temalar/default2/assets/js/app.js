...
var App = {
    ...
    initialize: function () {
        ...
        $('a[id^="switch_items_"]').on('click', function (e) {
             var server = $(this).attr('id').split('_')[2];
            e.preventDefault();
            App.loadLatestItems(server);
        });
    ...
    loginUser: function (data) {
        ...
        if (typeof(data) != "undefined") {
            dataToSend = '&' + $.param({'username': App.user_info[0], 'password': App.user_info[1], 'server': data, 'dmn_csrf_protection': DmNConfig.dmn_csrf_token});
        }
        else {
            dataToSend = '&' + $.param({'username': App.user_info[0], 'password': App.user_info[1], 'dmn_csrf_protection': DmNConfig.dmn_csrf_token});
        }
        $.ajax({
    ...
    populateRanking: function (rank, server, c_class) {
        if (typeof(c_class) != "undefined" && c_class != 'all') {
            dataToSend = '&' + $.param({'type': rank, 'server': server, 'class': c_class, 'dmn_csrf_protection': DmNConfig.dmn_csrf_token});
        }
        else {
            dataToSend = '&' + $.param({'type': rank, 'server': server, 'dmn_csrf_protection': DmNConfig.dmn_csrf_token});
        }
    ...
    populateSidebarRanking: function (rank, server, top) {
        dataToSend = '&' + $.param({'type': rank, 'server': server, 'top': top, 'dmn_csrf_protection': DmNConfig.dmn_csrf_token});
        $.ajax({
            type: 'POST',
            url: DmNConfig.base_url + 'rankings/load_ranking_data',
            data: dataToSend,
    ...
    loadLatestItems: function (server) {
        $.ajax({
			type: "POST",
            url: DmNConfig.base_url + 'market/latest-items',
            data: {ajax: 1, server: server, dmn_csrf_protection: DmNConfig.dmn_csrf_token},
            beforeSend: function () {
                App.showLoader();
            },
            complete: function () {
    ...
    changeServerInfo: function (key) {
		...
		$('#sActive').text($('#active-' + key).text()).css("color", "#837E7E");
                    ...
                    if (typeof(info) != 'undefined') {
                        ...
                        setTimeout(function () {
                            $.ajax({
                                ...
                                data: {item_hex: hex, ajax: 1, dmn_csrf_protection: DmNConfig.dmn_csrf_token},
                                success: function (data) {
                                    if (data.error) {
    ...
    serverDate: function () {
        ...
        $.ajax({
            type: "GET",
            url: DmNConfig.base_url + "ajax/get-time&dmn_csrf_protection="+DmNConfig.dmn_csrf_token,
            dataType: 'jsonp',
            async: false,
            success: function (data) {
...
var serverTime = {
    ...
    init: function (e, c) {
        ...
        f.eleServer = e;
        f.eleLocal = c;
        $.getJSON(DmNConfig.base_url + "ajax/get-time&callback=?&dmn_csrf_protection="+DmNConfig.dmn_csrf_token, function (a) {
