...
<div class="content clearfix" id="wt_refpoint">
    ...
    <div id="category">
        <h2>
            <ul class="breadcrumb">
                <li>
                    <a href="javascript:void(0);" title="En gözdeler"><?php echo ($kategori == FALSE) ? $baslik : $kategori->isim;?></a>
        ...
        <div class="tabbable tabs-left">
            <ul id="subnavi" class="nav nav-tabs">
                <?php foreach ($menu as $key => $row):?>
                    <li class="has-subnavi" >
                        <a href="<?php echo base_url('market/urunler/'.$row->KID);?>" class="btn-catitem btn-catitem<?php if($kategori != false AND $kategori->KID == $row->UKID){echo '-active';}?>">
            ...
            <div class="tab-content">
                ...
                <?php if($esyalarGrup) foreach ($esyalarGrup as $grupAdi => $grup): ?>
                <h3 class="clearfix" id="h3_sub_23835"><?php echo $grupAdi;?> <span class="item_count">(<?php echo (int) count($grup);?>)</span></h3>
                <ul class="item-list card clearfix">
                    <?php if($grup) foreach ($grup as $key => $row): ?>
                        <?php
                            $cokluSatis = 0;
                                        ...
                                        <div class="action-box left">
                                            <?php if ($cokluSatis > 1):?>
                                                <div class="zs-dropdown">
                                                    <button class="dropdown-toggle" type="button" data-toggle="dropdown">
                                                        <span class="dropdown-selection"><?php echo $row['adet_1'];?></span>
                                                        <span class="btn-default"><span class="caret"></span></span>
                                                    </button>
                                                    <ul class="dropdown-menu">
                                                        <?php for ($i=0;$i<$cokluSatis;$i++): ?>
                                                            <li onclick="changePrice2(<?php echo $row['EID']?>,<?php echo $i+1?>);">
                                                                <a class="dropdown-option" data-count="<?php echo $row['adet_'.($i+1)];?>" data-text="<?php echo $row['adet_'.($i+1)];?>">
                                                                    <span><?php echo $row['adet_'.($i+1)];?></span>
                                                                    <?php if ($row['indirim_'.($i+1)] > 0):?>
                                                                        <span class="extra-info">%<?php echo $row['indirim_'.($i+1)]?> İndirim</span>
                                                                    <?php endif;?>
                                                                </a>
                                                            </li>
                                                        <?php endfor;?>
...
<script>
    function changePrice2(item_id,num) {
        var url = "<?php echo base_url('market/ajax_ucret_hesapla');?>";
